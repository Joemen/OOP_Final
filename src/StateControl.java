...
public class StateControl {
    ...
    public static enum State{ORIGIN,START,ACTION_1,DEPLOY_1,ACTION_2,DEPLOY_2,WAR,WAREND,CLEAR,ENDROUND,ENDGAME}
    ...
    public static void control(State state){
        switch(state){
            case ORIGIN:
                ...
                break;
            case START:
                MainFrame.newGameMenuItem.setEnabled(false);
            ...
            case ACTION_1:
				...
				if(Game.round != 1)
					MainFrame.historyMenuItemLeft.setEnabled(true);
            ...
            case ACTION_2:
            	...
            	MainFrame.historyMenuItemLeft.setEnabled(false);
            	if(Game.round != 1)
            		MainFrame.historyMenuItemRight.setEnabled(true);
            	TotalPrint.printRound();
            ...
            case WAR:
            	...
            	MainFrame.historyMenuItemRight.setEnabled(false);
    ...
    public static void action(ActionEvent event){
        if(event.getSource() == MainFrame.btnPlayer_1 ){
            ...
            if(NewFrame.num == 1 ){
                ...
                drawHello = 0;
                EnemyAI.AI_index = false;
            }
            else if (NewFrame.num == 0 ){
                Game.player[0].setPlayerName(NewFrame.textField_1.getText());
                Game.player[1].setPlayerName("Liu Bei");               
                drawHello = 0;
                EnemyAI.AI_index = true;
            }
            if (NewFrame.num == 0 && NewFrame.player1roleChoice !=null && NewFrame.textField_1.getText().length()!=0){
            	Game.player[0].setRole(NewFrame.player1_role_num);
                Game.player[1].setRole(0);
            ...
            else if(NewFrame.num != 0 && NewFrame.currentChoice !=null && NewFrame.player1roleChoice !=null && NewFrame.player2roleChoice !=null && NewFrame.textField_1.getText().length()!=0 && NewFrame.textField_2.getText().length()!=0 ){
                Game.player[0].setRole(NewFrame.player1_role_num);
        	...
        	if(ActionFrame.action_flag == 1&&ActionFrame.ConstructionCombo.getSelectedIndex()!=-1){ // Construction
                ...
                new ConstructAnimation(ActionFrame.ConstructionCombo.getSelectedIndex());
                if((Game.turn+1)%2==0){
                   ...
                   // toasterManager.showToaster( new ImageIcon( "pic/gif/em15.gif" ), "Constructing tower!" );
                    System.out.println("ConstructionCombo");
                }
                ...
                new FactoryMunition(ActionFrame.MuntionCombo.getSelectedIndex());
                if((Game.turn+1)%2==0){
                    //Toaster toasterManager = new Toaster(MainFrame.scrollPane.getLocation().x +210 ,MainFrame.scrollPane.getLocation().y+95 );
                    // Show a simple toaster
                    //toasterManager.showToaster(  new ImageIcon( "pic/gif/em05.gif" ), "Constructing army!" );
                    System.out.println("MuntionCombo");
                ...
                if((Game.turn+1)%2==0){
                    Toaster toasterManager = new Toaster(MainFrame.scrollPane.getLocation().x +210 ,MainFrame.scrollPane.getLocation().y+95 );
                    // Show a simple toaster
                    toasterManager.showToaster(  new ImageIcon( "pic/gif/em10.gif" ), "Getting money!" );
                ...
                if((Game.turn+1)%2==0){
                    Toaster toasterManager = new Toaster(MainFrame.scrollPane.getLocation().x +210 ,MainFrame.scrollPane.getLocation().y+95 );
                    // Show a simple toaster
                    toasterManager.showToaster(  new ImageIcon( "pic/gif/em18.gif" ), "Exploring!" );
                    System.out.println("ExploreCombo");
                }
                else if((Game.turn+1)%2==1){
                    Toaster toasterManager = new Toaster(MainFrame.scrollPane.getLocation().x -150 + MainFrame.scrollPane.getWidth() ,MainFrame.scrollPane.getLocation().y+95 );
                    // Show a simple toaster
            ...
            // Computer.fight_UI(Game.tower[Game.player[(Game.turn+1)%2].getCampNum()-1], Game.tower[Game.player[(Game.turn)%2].getCampNum()-1], Game.player);
            drawHello = 1;
            Clickshake clickshake = new Clickshake("Jedi");
            clickshake.start();
        }
            ...
            try {
				FileWriter fileWritter = new FileWriter("history_Left.txt",true);
			    String myString1 = MainFrame.textArea_1.getText();
			    if(Game.round == 1){
			    	PrintWriter writer = new PrintWriter("history_Left.txt");
			    	writer.print("");
			    	writer.close();
            ...
            try {
				FileWriter fileWritter = new FileWriter("history_Right.txt",true);
			    String myString1 = MainFrame.textArea_2.getText();
			    if(Game.round == 1){
			    	PrintWriter writer = new PrintWriter("history_Right.txt");
			    	writer.print("");
			    	writer.close();
			    	fileWritter.append(myString1 + "\n");
			    }else{
